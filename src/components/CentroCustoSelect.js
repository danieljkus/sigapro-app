...
const Registro = ({ registro, onRegistroPress }) => {
    return (
        <Card containerStyle={{ padding: 0, margin: 7, borderRadius: 2, }}>
            <TouchableOpacity
                onPress={() => onRegistroPress(registro.contab_cc_codigo)}
            >
                ...
                <View style={{ paddingLeft: 20, paddingVertical: 4 }}>
                    <Text style={{ color: Colors.textPrimaryDark, fontSize: 15 }}>
                        {String(registro.contab_cc_descricao).trim()}
                    </Text>
                </View>
            </TouchableOpacity>
        </Card>
...
class CentroCustoSelect extends PureComponent {
    ...
    componentDidMount() {
        ...
        if (this.props) {
            this.setState({
                cc_select: {
                    contab_cc_codigo: this.props.contab_cc_codigo,
                    contab_cc_descricao: String(this.props.contab_cc_descricao).trim()
                },
            });
        }
    }
    ...
    onRegistroPress = (contab_cc_codigo) => {
        this.setState({
            codCC: contab_cc_codigo,
        });
        this.onAbrirBuscaModal(false);
        this.buscaRegistros(contab_cc_codigo);
    }
    ...
    render() {
        ...
        const { codCC, carregando, loading, refreshing, listaRegistros } = this.state;

        // console.log('CentroCustoSelect.this.props', this.props)
        // console.log('CentroCustoSelect.this.state', this.state)

        const codConta = codCC ? codCC : (value ? value.contab_cc_codigo : '');
        const descricao = value ? String(value.contab_cc_descricao).trim() : '';
        this.setState({ codCC: codConta });
                        ...
                        >
                            <View style={{ marginTop: 4 }}>
                                <FlatList
                                    ...
                                    keyExtractor={registro => registro.contab_cc_codigo}

...
export default class FichaViagemChegadaScreen extends Component {
    ...
    calculoKm = (man_fv_odo_fim, man_fv_qtde_comb, man_fv_qtde_arla) => {
        ...
        if ((man_fv_qtde_comb) && (man_fv_qtde_comb > 0)) {
            mediaComb = (parseFloat(kmRota) / (parseFloat(man_fv_qtde_comb) + parseFloat(man_fv_qtde_comb_extra))).toFixed(3);
        ...
        let mediaArla = 0;
        if ((man_fv_qtde_arla) && (man_fv_qtde_arla > 0)) {
            mediaArla = (parseFloat(kmRota) / parseFloat(man_fv_qtde_arla)).toFixed(3);
    ...
    onInputChangeVeiculo = (id, value) => {
        ...
        if (value) {
            this.setState({
                ...
                man_fv_qtde_comb_extra: value.combExtraRota.toFixed(0),
    ...
    onFormSubmit = (event) => {
        ...
        if (this.state.man_fv_qtde_comb > 600) {
            Alert.showAlert('Quantidade de combustível maior que o permitido');
            return;
        }

        if ((this.state.man_fv_qtde_comb > 0) && (this.state.man_fv_qtde_comb > this.state.veiculo_select.estoqComb)) {
            Alert.showAlert('Estoque de Combustível insuficiente');
            return;
        }

        if ((this.state.man_fv_qtde_arla > 0) && (this.state.man_fv_qtde_arla > this.state.veiculo_select.estoqArla)) {
